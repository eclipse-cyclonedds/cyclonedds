...
int LLVMFuzzerTestOneInput(void *data, size_t size)
{
    ...
    for(size_t i = 0; i < sizeof(fixed_types)/sizeof(fixed_types[0]); i++) {
        ...
        if (dds_stream_normalize(data_copy, (uint32_t) size, false, DDSI_RTPS_CDR_ENC_VERSION_1, &desc, false, &actual_size) == DDS_STREAM_NORMALIZE_SUCCESS) {
            void *sample = ddsrt_calloc(1, desc.size);
            dds_istream_t is;
            dds_istream_init(&is, actual_size, data_copy, DDSI_RTPS_CDR_ENC_VERSION_1);
